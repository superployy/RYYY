...
def _ydl_opts(extra: dict = None) -> dict:
    """Base yt-dlp options — OAuth2, no cookies, Android client."""
    opts = {
        'username': 'oauth2',
        'password': '',
        'token_file': config.ABSOLUTE_PATH + config.OAUTH_TOKEN_PATH,
        'extractor_args': {
            'youtube': {
        ...
        if song.info.title is None:
            ...
            try:
                downloader = yt_dlp.YoutubeDL(
                    _ydl_opts({'format': 'bestaudio', 'title': True})
                )
                r = downloader.extract_info(song.info.webpage_url, download=False)
            except Exception:
                await asyncio.sleep(1)
                downloader = yt_dlp.YoutubeDL(_ydl_opts({'title': True}))
                r = downloader.extract_info(song.info.webpage_url, download=False)
